...
const Steps = ({currStep, dictionary}:{currStep: number, dictionary: any}) => {
  ...
  return (
    <nav className="my-6 lg:my-[40px] flex items-center justify-between">
      {dict["steps"].map((step: any, i: number) => {
        ...
        return (
          <a key={`checkoutStep${i}`} href={`#step${i}`} className="step flex max-lg:flex-col items-center gap-2 lg:gap-[20px] text-black text-xs lg:text-xl">
            <span className={`relative flex items-center justify-center w-5 h-5 lg:w-10 lg:h-10 rounded-full text-sm lg:text-xl ${isNext ? 'text-crayola' : 'text-white'} ${isNext ? 'border border-crayola' : ''} ${isCurr ? 'bg-crayola' : (isNext ? 'bg-white' : 'bg-apple')}`} aria-hidden>
